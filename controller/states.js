...
exports.getStates=async (req, res, next)=>{
    try{
        ...
        res.status(200).json({success:true, message:"states fetched successfully", data:statesList, noOfRecords:statesList.length})
    }catch(err){
...
exports.getState=async (req, res, next)=>{
    try{
        ...
        if(!state){
            return res.status(404).json({success:false, message:"State not found", data:null})
        }
